...
namespace platform_connectors.twitch_connector {
    public static class Twitch_connectorModule {
        ...
        // Track last subscription attempt times to avoid rapid repeated creates (type -> timestamp)
        private static readonly System.Collections.Concurrent.ConcurrentDictionary<string, DateTime> _lastSubscriptionAttempt = new System.Collections.Concurrent.ConcurrentDictionary<string, DateTime>();
        ...
        public static void New(object? config = null, bool? is_bot_account = null) {
            // Minimal module-level initializer: store config values if provided
            try {
                if (config is System.Collections.Generic.Dictionary<string, object> dict) {
                    // persist known keys
                    if (dict.ContainsKey("oauth_token")) core.config.ConfigModule.SetPlatformConfig(core.platforms.PlatformIds.Twitch, "oauth_token", dict["oauth_token"]?.ToString() ?? string.Empty);
                    if (dict.ContainsKey("client_id")) core.config.ConfigModule.SetPlatformConfig(core.platforms.PlatformIds.Twitch, "client_id", dict["client_id"]?.ToString() ?? string.Empty);
                    if (dict.ContainsKey("bot_username")) core.config.ConfigModule.SetPlatformConfig(core.platforms.PlatformIds.Twitch, "bot_username", dict["bot_username"]?.ToString() ?? string.Empty);
                }
            } catch (Exception ex) {
                Console.WriteLine($"Twitch_connectorModule.New init error: {ex.Message}");
            }
            return;
        }
        ...
        public static void DeleteMessage(string? message_id) {
            try {
                Console.WriteLine($"Twitch_connectorModule.DeleteMessage called message_id={message_id}");
                // Notify any registered deletion callbacks so UI/state can update. This does not call Helix yet.
                if (_deletionCallbackDel != null) {
                    try { _deletionCallbackDel.DynamicInvoke(message_id); } catch { }
                }
            } catch (Exception ex) {
                Console.WriteLine($"Twitch_connectorModule.DeleteMessage error: {ex.Message}");
            }
        }
                    ...
                    if (string.Equals(msgType, "session_welcome", StringComparison.OrdinalIgnoreCase)) {
                        ...
                        if (root.TryGetProperty("payload", out var payload) && payload.TryGetProperty("subscription", out var sub)) {
                            ...
                            if (payload.TryGetProperty("event", out var ev)) {
                                ...
                                // If this is a chat message, also dispatch message-specific handlers
                                if (string.Equals(eventType, "channel.chat.message", StringComparison.OrdinalIgnoreCase)) {
                                    string? messageText = null;
                                    if (ev.TryGetProperty("message", out var mv) && mv.ValueKind == JsonValueKind.String) messageText = mv.GetString();
                                    // If messageText is present and not null/empty, invoke message callbacks
                                    if (!string.IsNullOrEmpty(messageText)) {
                                        OnMessageReceived(username, messageText);
                                        OnMessageReceivedWithMetadata(username, messageText, eventData);
                                    }
                                }
        ...
        public static void ParsePrivmsg(string? raw_message) {
            // Legacy IRC PRIVMSG parsing has been removed in favor of EventSub-first handling.
            // Keep a no-op placeholder to avoid accidental runtime calls.
            Console.WriteLine("ParsePrivmsg: legacy IRC parsing disabled (use EventSub events)");
            return;
        }
        ...
        private static async Task CreateEventSubSubscriptionIfNotExistsAsync(string type, string broadcasterId, string oauth, string clientId) {
            try {
                ...
                // Throttle repeated attempts for the same subscription type to avoid rapid retries
                var now = DateTime.UtcNow;
                var sessionKey = _eventSubSessionId ?? string.Empty;
                var compositeKey = type + "|" + sessionKey;
                var last = _lastSubscriptionAttempt.GetOrAdd(compositeKey, DateTime.UnixEpoch);
                if ((now - last) < TimeSpan.FromSeconds(1)) {
                    Console.WriteLine($"Skipping rapid re-attempt for EventSub type={type} session={sessionKey}");
                    http.Dispose();
                    return;
                }
                _lastSubscriptionAttempt[compositeKey] = now;
                ...
                if (postResp.IsSuccessStatusCode) {
                    ...
                    // Treat 409 Conflict as 'already exists' (idempotent)
                    if ((int)postResp.StatusCode == 409) {
                        Console.WriteLine($"Subscription create returned 409 for {type}, treating as already exists: {postTxt}");
                    } else if ((int)postResp.StatusCode == 401) {
                        // Unauthorized - may need reauth. Signal up via callback.
                        Console.WriteLine($"Subscription create unauthorized (401) for {type}: {postTxt}");
                        try { OnEventsubReauthRequested(null); } catch { }
                    } else {
    ...
    public class TwitchConnector {
        ...
        public void New(object? config = null, bool? is_bot_account = null) {
            try {
                // initialize instance-level recent message tracking
                if (this._recent_local_echoes == null) this._recent_local_echoes = new System.Collections.Generic.List<object>();
                if (this._recent_message_ids == null) this._recent_message_ids = new System.Collections.Generic.Dictionary<string, object>();
                if (this._recent_message_window == null) this._recent_message_window = 30.0;
                if (this._max_recent_message_ids == null) this._max_recent_message_ids = 200;
                this._initialized = true;
            } catch (Exception ex) {
                Console.WriteLine($"TwitchConnector.New error: {ex.Message}");
            }
        }

        // Original: def __init__(self, config=None, is_bot_account=False)
        public TwitchConnector(object? config = null, bool? is_bot_account = null) {
            // sensible defaults for instance fields
            this.worker_thread = false;
            this.worker = false;
            this.eventsub_worker_thread = false;
            this.eventsub_worker = false;
            this._last_worker_created = 0;
            this.config = config != null;
            this.is_bot_account = is_bot_account ?? false;
            ...
            this._recent_local_echoes = new System.Collections.Generic.List<object>();
            this._recent_message_ids = new System.Collections.Generic.Dictionary<string, object>();
            this._recent_message_window = 30.0;
            this._max_recent_message_ids = 200;
            this._initialized = true;
        ...
        public void SendMessage(string? message) {
            try {
                core.chat_manager.Chat_managerModule.SendMessageAsBot(core.platforms.PlatformIds.Twitch, message, true);
            } catch (Exception ex) {
    ...
    public class TwitchWorker {
        ...
        public void SetMetadataCallback(object? callback) {
            try {
                this._metadata_callback = callback;
                if (callback is Delegate d) {
                    Twitch_connectorModule.SetMetadataCallback(d);
                } else {
                    Twitch_connectorModule.SetMetadataCallback(null);
                }
            } catch (Exception ex) {
                Console.WriteLine($"TwitchWorker.SetMetadataCallback error: {ex.Message}");
            }
        }
        ...
        public void SetDeletionCallback(object? callback) {
            try {
                this._deletion_callback = callback;
                if (callback is Delegate d) {
                    Twitch_connectorModule.SetDeletionCallback(d);
                } else {
                    Twitch_connectorModule.SetDeletionCallback(null);
                }
            } catch (Exception ex) {
                Console.WriteLine($"TwitchWorker.SetDeletionCallback error: {ex.Message}");
            }
        }
        ...
        public void Run() {
            try {
                this.running = true;
                // start module-level EventSub websocket loop in background
                ...
                this.running = false;
                Console.WriteLine($"TwitchWorker.Run error: {ex.Message}");
        ...
        public async Task ConnectToTwitch() {
            try {
                await Twitch_connectorModule.ConnectToTwitch().ConfigureAwait(false);
            } catch (Exception ex) {
                try { if (this.error_signal is Delegate d) d.DynamicInvoke(ex.Message); } catch { }
            }
            await Task.Yield();
        ...
        public async Task RefreshTokenIfNeeded() {
            try {
                await Twitch_connectorModule.RefreshTokenIfNeeded().ConfigureAwait(false);
            } catch (Exception ex) {
                try { if (this.error_signal is Delegate d) d.DynamicInvoke(ex.Message); } catch { }
            }
            await Task.Yield();
        ...
        public async Task HealthCheckLoop(object? websocket) {
            // delegate to module-level loop where applicable
            try {
                await Twitch_connectorModule.HealthCheckLoop(websocket).ConfigureAwait(false);
            } catch (Exception) {
                await Task.Delay(TimeSpan.FromSeconds(30)).ConfigureAwait(false);
            }
            return;
        ...
        public async Task Authenticate() {
            try {
                await Twitch_connectorModule.Authenticate().ConfigureAwait(false);
            } catch (Exception ex) {
                try { if (this.error_signal is Delegate d) d.DynamicInvoke(ex.Message); } catch { }
            }
            await Task.Yield();
        ...
        public async Task HandleMessage(string? raw_message) {
            try {
                ...
                if (raw_message.StartsWith("{") || raw_message.StartsWith("[")) {
                    await Twitch_connectorModule.HandleMessage(raw_message).ConfigureAwait(false);
                } else {
                    // EventSub-first: ignore legacy IRC-style messages here
                    Console.WriteLine("TwitchWorker.HandleMessage: ignoring non-EventSub legacy IRC message");
                }
            } catch (Exception ex) {
                Console.WriteLine($"TwitchWorker.HandleMessage error: {ex.Message}");
            }
            await Task.Yield();
            return;
        }
        ...
        public void ParseClearmsg(string? raw_message) {
            try {
                Twitch_connectorModule.ParseClearmsg(raw_message);
            } catch (Exception ex) {
                Console.WriteLine($"TwitchWorker.ParseClearmsg error: {ex.Message}");
            }
        }
        ...
        public void ParseUsernotice(string? raw_message) {
            try {
                Twitch_connectorModule.ParseUsernotice(raw_message);
            } catch (Exception ex) {
                Console.WriteLine($"TwitchWorker.ParseUsernotice error: {ex.Message}");
            }
        }
        ...
        public void ParsePrivmsg(string? raw_message) {
            try {
                // Legacy: do not parse IRC PRIVMSG here. EventSub-first flow handles chat messages.
                Console.WriteLine("TwitchWorker.ParsePrivmsg: ignored legacy IRC PRIVMSG (EventSub-first)");
            } catch (Exception ex) {
                Console.WriteLine($"TwitchWorker.ParsePrivmsg error: {ex.Message}");
            }
        }
        ...
        public void Stop() {
            try {
                this.running = false;
                Twitch_connectorModule.Stop();
                try { if (this.status_signal is Delegate d) d.DynamicInvoke(false); } catch { }
            } catch (Exception ex) {
                Console.WriteLine($"TwitchWorker.Stop error: {ex.Message}");
            }
        }
        ...
        public async Task SendMessageAsync(string? message) {
            try {
                if (string.IsNullOrEmpty(message)) return;
                await Twitch_connectorModule.SendMessageAsync(message).ConfigureAwait(false);
            } catch (Exception ex) {
                Console.WriteLine($"TwitchWorker.SendMessageAsync error: {ex.Message}");
            }
            await Task.Yield();
            return;
        }
