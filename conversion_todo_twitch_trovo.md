...
Risk / Notes
- `twitch_connector.cs` contains many TODOs but already implements key HTTP-backed flows (ResolveUserIdAsync, ValidateToken, EventSub subscription creation). Focus implementation on parsing/EventSub/local echo and worker lifecycle to enable chat integration.
